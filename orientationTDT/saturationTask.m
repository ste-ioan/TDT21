...
%         j = 11:19;
        %targetpos = 289;
       targetpos = [249 269 289 309 329];    %group2
%                  targetpos = [213 233 253 273 293];    %group1    
    elseif satQuadrant == 2
%         j = 1:9;
        %targetpos = 61;
       targetpos = [25 43 61 79 97]; %group2
%                  targetpos = [65 83 101 119 137]; %group1
    end
%  i = 1:9; %group3
%  targetpos = [];
%  for ii = 1:numel(i)
%      for jj = 1:numel(j)
%          targetpos = [targetpos; i(ii)+(j(jj)-1)*19];
%      end
%  end
