...
fprintf(outfile, ['quadrant' delimiter 'satQuadrantYN' delimiter 'block' delimiter 'trial' delimiter 'targetalignment' delimiter 'targetorientation' delimiter 'SOA' delimiter]);
...
for k = 1:nblocks
    if k < 3
        ntrials = 40; % we have 5 SLOW trials per orientation-quadrant-alignment combination
    else
        ntrials = 160; % we have 20 trials per orientation-quadrant-alignment combination
    end
    ...
    for j = 1:ntrials
        ...
        if k < 3 && j < 20 % half trials of the first 2 blocks
            ...
            while LoopTime < resploop %after 'resploop' the loop will stop even without a response
                ...
                if keyIsDown == 1 % if a key is pressed (keyIsDown = 1)
                    ...
                    elseif keyCode(escKey)
                        fclose(outfile);
                        if logical(str2double(eyetracker)), stopeyelinkrecord;end
