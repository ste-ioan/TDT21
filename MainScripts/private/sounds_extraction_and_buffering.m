function [allsounds, Qsounds] = sounds_extraction_and_buffering(condition)

rng('shuffle')

%%%%%%%%%%%%%%% loads sounds on first execution %%%%%%%%%%%%%%
d = which('saturationTask.m');
f = strfind(d,'/');
soundPath = [d(1:f(end)) 'sounds/'];
privatePath = [d(1:f(end)) 'private'];
...
%     end
    ...
    cat1sounds(i).Y(cat1sounds(i).Y==0)=[];
...
for i=1:C1nsounds
    ...
    cat1sounds(i).soundBuffer = PsychPortAudio('Open');
    PsychPortAudio('FillBuffer',cat1sounds(i).soundBuffer, ...
        cat1sounds(i).Y');
end
...
for i=1:Dnsounds
    ...
    distractorsounds(i).soundBuffer = PsychPortAudio('Open');
    PsychPortAudio('FillBuffer',distractorsounds(i).soundBuffer, ...
        distractorsounds(i).Y');
end
...
for i=1:C2nsounds
    ...
    cat2sounds(i).soundBuffer = PsychPortAudio('Open');
    PsychPortAudio('FillBuffer',cat2sounds(i).soundBuffer, ...
        cat2sounds(i).Y');
end
...
if condition == 2
    ...
    for i=1:Qnsounds
        ...
        Qsounds(i).cue = 1;
        Qsounds(i).category = str2double(Qsounds(i).name(1:end-4));
        Qsounds(i).side = 'x';
        Qsounds(i).target = 0;
    end
    ...
    Qsounds(i).soundBuffer = PsychPortAudio('Open');
    PsychPortAudio('FillBuffer',Qsounds(i).soundBuffer, ...
        Qsounds(i).Y');
end
