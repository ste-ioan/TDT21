...
% taklen out Background buffer from side task to reduce crashing ç__ç
debugTime = false;
...
switch task
    ...
    case 'SIMON'
        if TaskDifficulty == 1
            ntrials = 80; % maybe 80 is enough
        else
            ntrials = 80; %from 80
            % since their length varies based on diff.
        end
...
% 
if Training
    ntrials = floor(ntrials/10);
    if ntrials <= 2
        ntrials = 3;
    elseif ntrials >6
        ntrials = 5;
    end % 3nback, 5 simon, 3side
end
...
%%%%%%% INSTRUCTIONS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
DrawFormattedText(wPtr,['Tâche: ' task], 'center', 'center', [128 128 128]);
Screen('Flip',wPtr);
if ~debug
    waitForSpaceKey;
end
DrawFormattedText(wPtr,'Prêt?\n Appuyez sur la barre d''espace pour démarrer l''expérience!', 'center', 'center', [128 128 128]);
Screen('Flip',wPtr);
if ~debug
    waitForSpaceKey;
end
...
%             if debug,msg = ['draws lines and square '];save(fullfile(path,'msglog.mat'),'msg');end
            if testScreen
                ...
                for id = 1:size_point(1)%aka for each line stempio
                    if newdrawpoint(id)%not sure why this passage is necessary, newdrawpoint is all 1's per line
...
%                         end
                ...
                if photodiodeFlag, Screen('FillRect',wPtr,round(col * c),[resolution(1)-diodeSquareSize resolution(2)-diodeSquareSize resolution(1) resolution(2)]);end
