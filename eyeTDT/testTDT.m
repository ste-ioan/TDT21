...
fprintf(outfile, ['quadrant' delimiter 'satQuadrant Y/N' delimiter 'block' delimiter 'trial' delimiter 'targetalignment' delimiter 'block_eye' delimiter 'satEyeYN' delimiter 'SOA' delimiter]);
...
if isempty(screenrect) % first run
    [screenrect]=psychInit(0,black,doubleScreenRatio);
...
if ~debug
    if any(str2double(subnumber)==1:8)
        eye_blocks = repmat({'L','R'},1,2);
    else
        eye_blocks = repmat({'R','L'},1,2);
    end
else
    eye_blocks = repmat({'L','R'},1,2);
end

% eye condition (saturation)
if ~debug
    left_eye_sat = [1:4:16,2:4:16];
    
    if any(str2double(subnumber) == left_eye_sat)
        satEye = 'L';
    else
        satEye = 'R';
    end
...
for k = 1:nblocks
    if eyelinkflag
        % eyelink setup at beginning of each block
        eyelinkInit(black,1)
        if ~debug
                WaitSecs(5);
    ...
    for j = 1:ntrials
        ...
        if ~debug
            ...
            if rand>.3
                keypressedFirst = keys(randi(2)); % randomly simulate F or J
            else
        ...
        satEyeYN = strcmp(satEye, eye_blocks{k});
        ...
        fprintf(outfile, ['%d' delimiter], satEyeYN);
